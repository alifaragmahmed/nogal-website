...
@endsection
@php 
    $builder = (new App\Chatbot)->getViewBuilder(); 
@endphp  
...
<div class="modal fade" tabindex="-1" role="dialog" id="addModal" style="width: 100%!important;height: 100%!important" >
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <center class="modal-title w3-xlarge">{{ __('add chatbot') }}</center>
      </div>
...
<div class="modal fade" tabindex="-1" role="dialog" id="editModal" style="width: 100%!important;height: 100%!important" >
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <center class="modal-title w3-xlarge">{{ __('edit chatbot') }}</center>
      </div>
...
$(document).ready(function() {
     $('#table').DataTable({
        ...
        "serverSide": true,
        "pageLength": 5,
        "ajax": "{{ url('/dashboard/chatbot/data') }}",
        "columns":[
            @foreach($builder->cols as $col)
            { "data": "{{ $col['name'] }}" },     
            @endforeach
